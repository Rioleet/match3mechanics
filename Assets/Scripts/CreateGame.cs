...
public class CreateGame : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < numCopies; i++)
        {
            ...
            {
                GameObject o = (GameObject) Instantiate(tile[j], new Vector3(-10, -10, 0), Quaternion.identity);
                o.SetActive(false);
                tileBank.Add(o);
                Debug.Log("Цвет = " + o.GetComponent<SpriteRenderer>().color);
            }
        }
        ShuffleList();
        ...
        for (int r = 0; r < rows; r++)
        {
            for (int c = 0; c < cols; c++)
            {
                ...
                //GameObject o = (GameObject)Instantiate(tile[UnityEngine.Random.Range(0,tile.Length)], tilePos, Quaternion.identity) as GameObject;
                //tiles[c, r] = new Tile(o, o.name);
            }
        }
    }
    void Update()
    {
        ...
        if(Input.GetMouseButtonDown(0))
        {
            ...
            if(hit)
            {
                ...
                //Debug.Log("tile1 = " + tile1.transform.position);
            }
        }
        //При отпускании пальца на другом тайле
        else if (Input.GetMouseButtonUp(0) && tile1)
        {
            ...
            if(hit)
            {
                ...
                //Debug.Log("tile2 = " + tile2.transform.position);
            }
            if(tile1 && tile2)
            {
                ...
                if(horzDist == 1 ^ vertDist == 1)
                {
                    ...
                    //Debug.Log("сброшены значения, временная позиция  = " + tempPos);
                    GetComponent<AudioSource>().Play();
                    matched = false;
                    //Сброс обратно значений
                    tile1 = null;
                    tile2 = null;
                }
            }
    ...
    void CheckGrid()
    {
        ...
        for (int r = 0; r < rows; r++)
        {
            ...
            {
                if (tiles[c, r] != null && tiles[c - 1, r] != null)  //если есть тайлы
                {
                    
                    if (tiles[c, r].type == tiles[c - 1, r].type)
                    {
                        counter++;
                        //Debug.Log("Совпадения!");
                    }
                    else counter = 1;//возращает по умолчанию
                    //Если нашлись 3, удаляет(по сути скрывает) их 
                    if (counter == 3)
                    {
                        matched = true;
                        if (tiles[c, r] != null)
    ...
    void RenewGrid()
    {
        ...
        for (int r = 1; r < rows; r++)
        {
            for (int c = 0; c < cols;c++)
            {
                if(r == rows-1 && tiles[c,r] == null) //Если вверху нет ничего
                {
                    Vector3 tilePos = new Vector3(c, r, 0);
                    for (int n = 0; n < tileBank.Count; n++)
                    {
                ...
                if (tiles[c,r] != null)
                {
                    //спускаем вниз, если снизу ничего нет
                    if(tiles[c,r-1] == null)
                    {
